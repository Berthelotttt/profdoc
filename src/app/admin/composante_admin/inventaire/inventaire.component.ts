...
export class InventaireComponent {
  ...
  user:Utilisateur[] = [];
    ...
    recuper_logiciel_prise_id_tech() {
      ...
      this.backend.getAllPrisemiseajourlogiciels().subscribe({
        next: (logiciels) => {
         this. logicielsFiltres =  logiciels
        },
        error: (err) => {
  ...
  recupe_all_logiciel_filtre_date() {
    const jour = 20; // Nombre de jours avant qu'une mise à jour soit nécessaire
    const dateActuelle = new Date();

    this.softfiltredate = this.softwareInfo.filter(software => {
      if (!software.installDate || software.installDate === "N/A") {
        return false; // Ignorer les logiciels sans date d'installation valide
      }

      const installDate = new Date(software.installDate);
      if (isNaN(installDate.getTime())) {
        return false; // Ignorer si la date n'est pas valide
      }
      ...
      // Retourner seulement si la différence dépasse 20 jours ET update_auto est false
      return differenceJours >= jour && software.update_auto === false;
    });

    console.log("Liste des logiciels qui doivent être mis à jour (update_auto = false):", this.softfiltredate);
  }
  ...
  prendre_logiciel(nom: string, idpc: string) {
    ...
    if (ordinateur) {
      ...
      const logiciel = ordinateur.logiciel.find(soft => soft['name'] === nom);
      ...
      if (logiciel) {
        ...
        this.date_dernier_instation=logiciel['installDate']
        logiciel['installDate'] = dateActuelle;
        ...
        this.backend.updateOrdinateur(ordinateur.id!, ordinateur).subscribe({
          next: (response) => {
            console.log("Ordinateur mis à jour dans le backend :", response.message);
            this.recupe_all_ordi()
  ...
  finition(id: number) {
    ...
    if (logiciel) {
      ...
      this.backend.updatePrisemiseajourlogiciel(id, logiciel).subscribe({
        next: (response) => {
  ...
  insertion_prisemiseajour(nom: string, idpc: string) {
    ...
    if (ordinateur) {
      ...
      if (logiciel) {
        ...
        this.backend.ajouterPrisemiseajourlogiciel(priseEnCharge).subscribe({
          next: (response) => {
